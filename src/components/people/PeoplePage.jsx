      ...
      <div className="w-full px-8 flex flex-col justify-center items-center text-slate-900">
        <div className="max-w-[72em] w-full flex flex-col">
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 sm:gap-y-6 md:gap-y-8 gap-x-4 mt-8">
              {
                people.map(person => 
...
function Person({name, titles, role, description, website, email, image}) {
  ...
  return (
    ...
    //     <div className="w-32 h-32 overflow-clip">
    //       <img className="w-full" src={`assets/people/${name.replaceAll(" ", "_").replaceAll(".", "").replaceAll("'", "").toLowerCase()}.jpg`}/>
    //     </div>
    //     <div className="flex flex-col max-w-64 sm:max-w-36 md:max-w-57">
    //       <div className="flex w-full">
    //         <p className="text-sm font-bold text-blue-800">{role}</p>
    //       </div>
    //       <div className="w-full mt-4">
    //         <h1 className="text-lg">{name}{titles.length !== 0 ? "," : ""} {titles.join(", ")}</h1>
